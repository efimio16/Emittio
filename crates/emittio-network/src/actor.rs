...
impl NetworkActor {
    ...
    #[command]
    async fn query(&mut self, query: Bytes, #[callback] callback: Result<Bytes, NetworkError>) {
        let payload_id = OsRng.next_u64();

        let peer = todo!("Find random peer");

        ok_or_reply!(callback, {
            let session = self.select_session(&peer).await?;

            // let frame = session.send(&FrameData::Query(payload_id, query))?;
            let frame = todo!();
        ...
        self.callbacks.insert((peer.id, payload_id), callback);
    }
