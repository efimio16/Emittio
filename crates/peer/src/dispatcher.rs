...
pub(super) enum PeerTableCmd {
    GetAllIds { tx: oneshot::Sender<Vec<PeerId>> },
    GetPeer { peer_id: PeerId, tx: oneshot::Sender<Option<Peer>> },
    AddPeer { peer: Peer },
}
...
impl PeerTableDispatcher {
    pub async fn get_all_ids(&self) -> Result<Vec<PeerId>, ChannelError> {
        ...
        self.tx.send(PeerTableCmd::GetAllIds { tx }).await.map_err(|_| ChannelError::Closed)?;
        rx.await.map_err(|_| ChannelError::Closed)
    }
    pub async fn get_peer(&self, peer_id: PeerId) -> Result<Option<Peer>, ChannelError> {
        let (tx, rx) = oneshot::channel();
        self.tx.send(PeerTableCmd::GetPeer { peer_id, tx }).await.map_err(|_| ChannelError::Closed)?;
        rx.await.map_err(|_| ChannelError::Closed)
    }
